...
SESSION_TIMEOUT = 600  # Hard cap (seconds) before a stuck session is aborted
...
if __name__ == "__main__":
    ...
    # Persistent across sessions — set once from model prep, reused on reconnect.
    ...
    while True:
        if session_num > 0:
            print(f"\n{'='*60}")
            print(f"[Server] Session {session_num} complete. Waiting for devices to reconnect...")
            print(f"{'='*60}\n")
        ...
        config = {"file_path": file_cfg,
                  ...
                  "skip_model_transmission": cached_ok,
        ...
        for i in range(len(devices)):
            t = threading.Thread(
                ...
                kwargs={"abort_event": session_abort},
            )
            threads.append(t)
        ...
        # Join all threads with a hard cap; abort if any are stuck past SESSION_TIMEOUT.
        ...
        if any(t.is_alive() for t in threads):
            print(f"[Server] Session timed out after {SESSION_TIMEOUT}s — setting abort to unblock threads.")
            session_abort.set()
            for t in threads:
                t.join(timeout=15)
    ...
    # Unreachable in normal operation; kept for clean shutdown if the loop is ever broken.
    send.close()
    context.term()
