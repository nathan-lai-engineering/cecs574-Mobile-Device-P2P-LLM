...
Start with TinyLlama for a first test run.
...
python device_client.py \
    --role worker \
    --ip 100.x.x.2 \        # THIS worker's own Tailscale IP
...
1. Set `config.properties` before building. The file is at:
   ```
   android/distributed_inference_demo/distribute_ui/src/main/assets/config.properties
   ```
   ```properties
...
The first run will be extremely slow because it has to transfer to the shard file over to the workers. This is a weakness of Lingualinked, but once transferred, the worker node will hold onto it.
...
2. **Model caching is auto-detected on subsequent runs.** The coordinator checks whether ONNX shards already exist for the current device count (`cached_ok` in `root.py`). On the first run the shards are always generated and sent. If you change the number of workers, delete the `onnx_model/to_send/` directory so shards are regenerated.

3. **Workers must register before the header.** The coordinator stops accepting registrations `TIMEOUT` seconds after the last device connects (`TIMEOUT = 10` at the top of `root.py`). If the header connects before all workers, the optimizer will see fewer devices than intended.

4. **APK `config.properties` must be set before building.** The file is baked into the APK at build time — changing it after installation requires a rebuild and reinstall. File location: `android/distributed_inference_demo/distribute_ui/src/main/assets/config.properties`.
